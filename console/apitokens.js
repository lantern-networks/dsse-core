...
// Backend (control plane): GET /admin/api-tokens -> {tokens:[…]}, POST /admin/api-tokens {name,role},
...
async function renderTokList(host) {
  ...
  const filtered = tokens.filter((t2) => !q || (t2.name || "").toLowerCase().includes(q) || (t2.role || "").toLowerCase().includes(q));
...
async function loadRoleOptions() {
  try {
    ...
    const cat = r && r.ok && r.body;
    const roles = (cat && (cat.roles || cat.assignable_roles)) || [];
    const opts = roles.map((x) => (typeof x === "string" ? x : (x.role || x.name || x.id))).filter(Boolean);
    if (opts.length) return opts.map((v) => ({ value: v, label: v }));
  } catch (e) { /* fall back */ }
  return [{ value: "admin", label: "admin" }, { value: "viewer", label: "viewer" }];
}
...
async function openTokenForm(content) {
  ...
  const roleF = uiField({ name: "role", label: bl({ en: "Role", ja: "ロール" }), type: "select", value: "", options: await loadRoleOptions(), hint: bl({ en: "What this token is allowed to do.", ja: "このトークンに許可する操作の範囲。" }) });
  ...
  submit.addEventListener("click", async () => {
    ...
    try {
      const r = await apiFetch("POST", "/admin/api-tokens", { name: nameF.get(), role: roleF.get() }, _TOK_PLANE);
...
function showSecretOnce(body, title) {
  const secret = body && (body.token || body.secret || body.api_token || body.value);
  if (!secret) { uiToast(title, "ok"); return; }
...
async function rotateToken(id, host) {
  ...
  if (!ok) return;
  const r = await apiFetch("POST", "/admin/api-tokens/" + encodeURIComponent(id) + "/rotate", {}, _TOK_PLANE);
...
async function revokeToken(id, name, host) {
  ...
  if (!ok) return;
  const r = await apiFetch("POST", "/admin/api-tokens/" + encodeURIComponent(id) + "/revoke", {}, _TOK_PLANE);
  if (!r.ok) { uiToast((r.body && (r.body.error || r.body.message)) || ("HTTP " + r.status), "err"); return; }
