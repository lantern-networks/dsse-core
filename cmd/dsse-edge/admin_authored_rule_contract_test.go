...
func TestAuthoredRuleHTTPPersistenceCompilationAndAudit(t *testing.T) {
	...
	p := &riskAuditPersister{base: blobstore.FilePersister{Path: filepath.Join(t.TempDir(), "rules.json")}}
...
func TestAuthoredRuleSyncRetriesSameGenerationAfterSaveFailure(t *testing.T) {
	...
	if _, err := src.apply(configBundlePayload{Rules: &authoredRuleBundle{}}, configApplyTargets{policyStore: policy.NewStore(nil)}); err == nil {
		t.Fatal("missing rule target acknowledged")
	}
}
