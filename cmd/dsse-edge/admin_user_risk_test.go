...
func TestUserRiskWriteRequiresScope(t *testing.T) {
	...
	overlay := revocation.NewHighRiskOverlay()
	auth := seedAdminConnectorAPITokenAuth("risk-reader", "risk-reader-bearer", []string{"admin.risk.read"})
	handler := newServerWithConfig(serverConfig{Evaluator: testEvaluator(), AdminAuth: auth, HumanIdentities: directory, HighRiskOverlay: overlay})
	request := func(method, body string) *httptest.ResponseRecorder {
		r := httptest.NewRequest(method, "/admin/risk-signals?entity_type=user", strings.NewReader(body))
		r.Header.Set("Authorization", "Bearer risk-reader-bearer")
		w := httptest.NewRecorder()
		handler.ServeHTTP(w, r)
		return w
	}
	if w := request("POST", `{"entity_type":"user","entity_id":"alice","severity":"high"}`); w.Code != http.StatusForbidden {
		t.Fatalf("read-only principal could write risk: %d", w.Code)
	}
	if _, ok := overlay.UserSeverity("tenant_lab_001", "alice"); ok {
		t.Fatal("denied write changed user risk")
	}
	if w := request("GET", ""); w.Code != http.StatusOK {
		t.Fatalf("read scope denied: %d", w.Code)
	}
}
