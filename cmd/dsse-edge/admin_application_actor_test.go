...
func TestApplicationLifecycleActor(t *testing.T) {
	for _, selected := range []bool{false, true} {
		t.Run(map[bool]string{false: "own", true: "selected"}[selected], func(t *testing.T) {
			...
			for _, rows := range [][]model.AuditLog{readSiteActorAudits(t, writer), outbox.insertedAudits} {
				count := 0
				for _, a := range rows {
				...
				if count != 5 {
					t.Fatalf("domain audits %d, want 5", count)
				}
			}
		})
	}
...
func TestApplicationAuditDoesNotInventActor(t *testing.T) {
	for _, r := range []*http.Request{nil, httptest.NewRequest("POST", "/admin/applications", nil)} {
		got := applicationAuditWithActor(r, model.AuditLog{TenantID: "target"})
		if got.ActorUserID != nil || got.TenantID != "target" {
			t.Fatal("invented actor or changed target tenant")
		}
	}
}
