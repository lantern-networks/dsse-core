...
func capacityRecord(tenant, id string, now time.Time) model.HumanApprovalEvent {
	expiry := now.Add(time.Hour).Format(time.RFC3339)
	return model.HumanApprovalEvent{ID: id, TenantID: tenant, ApprovalResult: "approved", ApproverUserID: stringPtr("approver"), ActorNHIID: stringPtr("agent"), ActionType: stringPtr("read"), CreatedAt: now.Format(time.RFC3339), ExpiresAt: &expiry}
}
func TestCapacityRetainsTerminalAuthorizationAcrossRestarts(t *testing.T) {
	for _, terminal := range []string{"denied", "expired", "revoked"} {
		t.Run(terminal, func(t *testing.T) {
			...
			if terminal == "revoked" {
				_, found, err := s.RevokeForTenant("a", "retained", "test")
				if err != nil || !found {
					t.Fatalf("revoke: %v", err)
				}
			} else {
				next := original
				next.ApprovalResult = terminal
			...
			for cycle := 0; cycle < 3; cycle++ {
				...
				if !ok || stored.ApprovalResult != terminal || IsActive(stored, now) || s.Count() != 1 {
...
func TestCapacityLoweringPreservesLoadedRecordsAndExistingMutations(t *testing.T) {
	...
	if !ok || saved.ApprovalResult != "revoked" || raised.Count() != 4 {
