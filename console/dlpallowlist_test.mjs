...
async function fixture(specs=[]){
 ...
 const context=vm.createContext({el,bl:x=>x.en,uiState(){},freshRender:host=>{const n=host.sequence=(host.sequence||0)+1;return ()=>host.sequence===n},emptyBox:()=>el('div'),simpleTable:()=>el('table'),uiBadge:()=>el('span'),uiConfirm:async()=>true,
  ...
  apiFetch:async(method,path,body)=>{if(path==='/admin/tenant')return {ok:true,status:200,body:{tenant_id:'tenant'}};if(method==='GET')return {ok:true,status:200,body:{tenant_id:'tenant',values:specs}};writes.push(body);return {ok:true,status:200,body:{tenant_id:'tenant',values:body.values}}}
 });
 vm.runInContext(readFileSync(new URL('./dlplibrary.js',import.meta.url),'utf8'),context);vm.runInContext(source,context);await context.renderDLPAllowlistView(el('div'));await tick();
...
test('blank or duplicate values do not submit, but literal case variants remain distinct',async()=>{
 ...
 f.fields.value.value='bluefin';await save.onClick();await tick();assert.equal(f.writes.length,1);assert.deepEqual(Array.from(f.writes[0].values),['BLUEFIN','bluefin']);
});
