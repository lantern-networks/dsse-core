...
func TestCandidateWritesUseControlPlaneAuthority(t *testing.T) {
	for _, operation := range []string{"create", "review", "materialize", "refresh", "publish"} {
		for _, sourced := range []bool{true, false} {
			...
			t.Run(name, func(t *testing.T) {
				...
				h := newServerWithConfig(serverConfig{Evaluator: testEvaluator(), AdminAuth: newAdminAuthStore(), Registry: connector.NewRegistry(), Writer: writer, AdminAuditOutbox: outbox, ApplicationCatalogStore: apps, PolicyCandidateStore: candidates, ConfigSourceURL: source})
