...
Treat an unavailable store as requiring investigation. A storage error may mean nothing was saved, or that saving completed but its acknowledgement was lost. Reconcile the saved state and the failed operation before restarting; a restart alone does not establish that a failed revocation was persisted. Existing successfully issued device certificates are not revoked by this token-store safeguard. The in-memory store without a persister remains non-durable. A completed, synced in-place save (`ErrSavedWithoutAtomicity` alone) retains its saved-but-not-atomic behavior and warning. Replacement with an unconfirmed flush (`ErrDurabilityUnconfirmed`) is a failure even when the compatibility error also matches `ErrSavedWithoutAtomicity`: no newly issued secret is returned for that save, the change is not published in memory, and further token operations stop. A device whose token spend could not be confirmed receives the same generic refusal as other invalid-token requests, with no certificate or admission. Restoring storage access alone does not clear the process's failure state.

If a batch enrolment-token issuance stops after creating some tokens, the Console shows the returned tokens once with the requested and returned counts. Save them before closing the dialog and investigate the failure before issuing more. A partially returned batch is not a full success or proof that the token store is currently available.
