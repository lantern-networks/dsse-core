...
|---|---|---|
| Tenant removal | Confirm tenant deletion and purge-order storage before changing live records or starting local erasure; classify PostgreSQL edit failures as unavailable storage | File failure/retry and tombstone/order reload tests; authenticated deletion HTTP failure/retry, existing purge refusal regressions and race checks. SQL error classification used a closed handle, not a live PostgreSQL deployment. No new deletion GUI acceptance is claimed |
