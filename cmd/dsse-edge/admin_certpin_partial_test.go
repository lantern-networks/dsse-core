...
func newCertPinPartialFixture(t *testing.T, candidateP, assetP, ruleP blobstore.Persister, source string) *certPinPartialFixture {
	...
	apply := newTenantInspectionApplier(f.engine, inspectionposture.NewStore(), f.rules, f.assets, knownbypass.NewOverrideStore(), func() []knownbypass.Group { return nil }, []string{"*"}, nil)
	apply("")
	...
	for _, pathType := range []string{"manual", "materialize"} {
		for _, stage := range []string{"bypass_endpoint", "bypass_rule"} {
			t.Run(pathType+"/"+stage, func(t *testing.T) {
				...
				if f.applies != 0 || !f.inspect(f.tenant) || !f.inspect("other") || len(f.rules.Snapshot()) != 0 {
				...
				w = f.post(path, body)
				if w.Code != 200 || f.applies != 1 || f.inspect(f.tenant) || !f.inspect("other") {
...
func TestCertPinExistingBypassAfterUnconfirmedRegistration(t *testing.T) {
	...
	if f.inspect(f.tenant) || !f.inspect("other") || f.applies != 1 {
