...
async function fixture(specs=[]){
 ...
 const context=vm.createContext({el,bl:x=>x.en,uiState(){},emptyBox:()=>el('div'),simpleTable:()=>el('table'),uiBadge:()=>el('span'),uiConfirm:async()=>true,
  ...
  apiFetch:async(method,path,body)=>{if(method==='GET')return {ok:true,body:{datasets:specs}};writes.push(body);return {ok:true,body:{dataset:{name:body.name,count:1},datasets:[{name:body.name,count:1}]}}}
 });
 vm.runInContext(source,context);await context.renderDLPFingerprintsView(el('div'));await tick();
...
test('dataset save suppresses repeated submission and keeps the error in the editor for retry',async()=>{
 ...
 f.context.apiFetch=async(_method,_path,body)=>{f.writes.push(body);return {ok:true,body:{dataset:{name:body.name,count:1},datasets:[{name:body.name,count:1}]}}};await save.onClick();
