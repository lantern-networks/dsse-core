...
func (s *Store) ApplyReport(ctx context.Context, tenantID, reporter string, seq uint64, flows []FlowObservation, now time.Time) (applied bool, err error) {
	...
	if s.receipts[reporter].Contains(seq) {
		if s.dirty {
			return false, s.saveReportLocked(s.flows, s.receipts)
		}
		return false, nil
	}
	next := cloneFlows(s.flows)
	if err := mergePending(next, map[string]map[string]FlowObservation{tenantID: add}); err != nil {
		return false, err
	}
	receipts := make(map[string]ReportReceipt, len(s.receipts)+1)
	for key, value := range s.receipts {
		receipts[key] = value
	}
	receipts[reporter] = nextReceipt
	pruneReceipts(receipts, now)
	(&Store{flows: next, retention: s.retention}).pruneLocked(now)
	if err := s.saveReportLocked(next, receipts); err != nil {
		return false, err
	}
	return true, nil
}

// Counts and receipts become visible together. A known replacement is retained
// after an unconfirmed flush, but retries must confirm it before acknowledging.
func (s *Store) saveReportLocked(next map[string]map[string]FlowObservation, receipts map[string]ReportReceipt) error {
	if s.persister != nil {
		data, err := encodeRow(next, receipts)
		if err == nil {
			err = blobstore.UnconfirmedSave(s.persister.Save(data))
		}
		if err != nil {
			if errors.Is(err, blobstore.ErrDurabilityUnconfirmed) {
				s.flows, s.receipts = next, receipts
				s.dirty = true
			}
			return err
		}
	}
	s.flows, s.receipts, s.dirty = next, receipts, false
	return nil
}
