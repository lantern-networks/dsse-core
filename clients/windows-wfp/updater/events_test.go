...
//
// So the next person adding an event to dsse-updater gets a failing test rather than a silently unreadable event.
func TestEveryEventIDIsInTheRangeWindowsCanRender(t *testing.T) {
	...
	for name, id := range map[string]uint32{
		...
		"evtProgress":        evtProgress,
		"evtExecuting":       evtExecuting,
		"evtRefused":         evtRefused,
		"evtRefusedManifest": evtRefusedManifest,
		"evtStageFailed":     evtStageFailed,
		"evtInterrupted":     evtInterrupted,
		"evtBlocked":         evtBlocked,
