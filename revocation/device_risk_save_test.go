...
func TestLegacyDeviceRiskPreservesEscalationButRefusesUnsavedDowngrade(t *testing.T) {
	...
	p.err = nil
	if _, err := o.SetUserRisk(UserRisk{TenantID: "foreign", ID: "user", Severity: "high"}); !errors.Is(err, ErrRiskUnavailable) {
		t.Fatal("unconfirmed legacy escalation requires explicit retry", err)
	}
	o.Mark("owned", "high")
