...
func TestLegacyRestrictionPartialSaveAndRetry(t *testing.T) {
	runtime, bundle, s := legacyRestrictionFixture(t)
	...
	var partial *TenantRestrictionUpdateError
	if !errors.As(err, &partial) || partial.Stage != "runtime_status_save" || !resp.PersistedToOperatorConfig || !resp.PersistedToValueStore || len(resp.AppliedRefs) != 1 || len(resp.AppliedSaaS) != 0 || resp.HotApplied {
	...
	for _, path := range []string{runtime.TenantRestrictionOperatorConfigPath, runtime.TenantRestrictionOperatorValueStorePath} {
		...
		if v, _ := restored.ResolveHeaderValue("operator_config_ref:allowed"); v != "new.example" {
			t.Fatal("confirmed file lost")
		}
	}
