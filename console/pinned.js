...
//
// Backend: candidate reads/reviews use the observation store; bypass registration uses the control plane.
// GET /admin/policy-candidates, POST /admin/policy-candidates/{id}/review
...
function renderPinnedSitesView(content) {
  ...
  const submitAdd = async () => {
    ...
    try {
      ...
      addStatus.textContent = bl({ en: host + " — registration saved. Check the serving Edge below.", ja: host + " の登録を保存しました。下のEdgeの状態も確認してください。" });
      await reloadAll();
    } catch (e) {
