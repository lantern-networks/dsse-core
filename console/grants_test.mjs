...
test('partial revoke offers retry; retry does not reconfirm and clears only its notice',async()=>{const f=fixture();f.mockRefresh();let partial=true;f.context.apiFetch=async()=>({ok:!partial,status:partial?500:200,body:{grant_id:grant.grant_id,tenant_id:'tenant',status:partial?'partial':'revoked',applied:true,...(partial?{persistence:'unconfirmed'}:{})}});await f.context.revokeGrant(grant,f.host,'Alice');assert.equal(f.host.__grantNotices.size,1);assert.match([...f.host.__grantNotices.values()][0].message,/before restarting/);assert.equal(f.toasts.length,0);partial=false;await f.context.revokeGrant(grant,f.host,'Alice',true);assert.equal(f.confirms.length,1);assert.equal(f.host.__grantNotices.size,0);assert.equal(f.toasts.length,1);assert.match(f.toasts[0].message,/Alice/)});

test('a completed non-atomic save shows a storage warning without offering a redundant retry',async()=>{const f=fixture();f.mockRefresh();f.context.apiFetch=async()=>({ok:true,status:200,body:{status:'revoked',grant_id:grant.grant_id,tenant_id:grant.tenant_id,persistence:'saved_non_atomic'}});await f.context.revokeGrant(grant,f.host,'Alice');f.context.drawGrantNotices(f.host);assert.match(f.host.textContent,/This save completed/);assert.equal(f.host.querySelectorAll('button').length,0);assert.equal(f.toasts.length,0)});
test('unknown or contradictory persistence results never announce confirmed revocation',()=>{const f=fixture();for(const persistence of ['unconfirmed','unknown',null,{}])assert.throws(()=>f.context.grantRevokeOutcome({ok:true,status:200,body:{status:'revoked',grant_id:grant.grant_id,tenant_id:grant.tenant_id,persistence}},grant),/unconfirmed/)});
