...
|---|---|---|
| Tenant edits | Keep omitted settings and customer delegation withdrawal when changing tenant details; record the authenticated administrator for self-service settings edits | Name-only and explicit-clear HTTP tests, saved-state reload and protected delegation tests; local operator browser edit/reload with stored timezone, delegation withdrawal and actor audits preserved. Multi-CP session propagation remains a separate check |
