...
// Edges send observation deltas to the control plane on a dedicated shipper so
// delayed inventory writes do not block audit delivery. Counts and receipts are
// stored together; retries within the receipt window do not add a report twice.
// Each process uses a new reporter ID, and each stream has its own sequence.
//
// The reporter flushes periodically and on graceful shutdown. Abrupt termination
// can lose unflushed observations, including observations retained after a local
// write failure. The shipper uses a bounded spool; this is not lossless delivery.
...
const (
	...
	// Maximum queued distinct observations; reaching this also requests an early flush.
...
// restart-durability: bounded_buffer — up to 5000 queued distinct observations,
// plus up to 5000 in the current flush. The dedicated spool and CP own emitted records.
// Unflushed counts live in memory. Write failures retain them for the next flush;
// an abrupt stop can lose that pending batch.
...
// Caller holds mu. Drops are explicit health data, never a silent success claim.
func (r *observationReporter) admitPendingLocked(count uint64) bool {
...
func (r *observationReporter) eastWestFlow(tenantID, source, user, destination, serviceFamily string, port int, now time.Time) {
	...
	if !ok {
		if !r.admitPendingLocked(uint64(1)) {
			return
		}
...
func (r *observationReporter) restoreEastWest(tenant string, flows []eastwestobserve.FlowObservation) {
	...
	for _, f := range flows {
		...
		if !ok {
			if !r.admitPendingLocked(uint64(f.Count)) {
				continue
			}
...
func (r *observationReporter) restoreCandidates(tenant string, obs []policycandidate.ReportedObservation) {
	...
	for _, c := range obs {
		...
		if !ok {
			if !r.admitPendingLocked(uint64(c.Count)) {
				continue
			}
...
// stopObservationReports attempts a final flush before closing the shipper spool.
