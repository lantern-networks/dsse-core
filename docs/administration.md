...
The program selection is locked during download. Closing the dialog or changing the active organization, credentials or authority discards late results. Download retries do not issue enrollment credentials; opening Add connector again remains a separate enrollment operation. This browser check does not add a publisher signature, make catalogue publication atomic, or verify programs fetched directly outside the Console. The catalogue names the requesting tenant and each program's resolved source (`tenant` or `deployment`). Downloads must return matching `X-Dsse-Connector-Program-Tenant` and `X-Dsse-Connector-Program-Source` headers, plus the catalogue's `X-Artifact-Sha256`, before the Console checks the actual bytes. A scope mismatch can indicate a changed publication source even when the bytes are identical; check the selected organization and reload the list. The existing tenant override and shared fallback lookup order is unchanged. These response checks are not a server-side request scope pin or a transaction across publication and download.

Update the control plane before or together with the Console. Older catalogues without tenant/source fields or downloads without the response headers cannot be verified and are refused by the updated Console. With a scoped API token and no browser session, the catalogue's tenant is trusted under server authorization; no additional tenant-model read permission is required. Opening Add connector again to reload a successful catalogue is a separate enrollment operation and can issue a new credential.
