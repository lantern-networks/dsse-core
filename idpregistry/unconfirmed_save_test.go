...
func TestUnconfirmedSaveCannotReportSuccess(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			candidate := sampleConn("tenant_a", "target")
			candidate.DisplayName = "Original"
			if _, err := s.Upsert(candidate); err != nil {
				t.Fatal(err)
			}
			candidate.DisplayName = "Changed"
			mutate := func() error { _, err := s.Upsert(candidate); return err }
			view := func(s *Store) string { c, _ := s.Get("tenant_a", "target"); return c.DisplayName }
