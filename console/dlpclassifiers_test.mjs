...
async function fixture(specs=[]){
 ...
 const context=vm.createContext({el,bl:x=>x.en,uiState(){},emptyBox:()=>el('div'),simpleTable:()=>el('table'),uiBadge:()=>el('span'),uiConfirm:async()=>true,
  ...
  apiFetch:async(method,path,body)=>{if(method==='GET')return {ok:true,body:{classifiers:specs}};writes.push(body);return {ok:true,body:{classifiers:body.classifiers}}}
 });
 vm.runInContext(source,context);await context.renderDLPClassifiersView(el('div'));await tick();
...
test('identifier save suppresses repeated submission and keeps the error in the editor for retry',async()=>{
 ...
 f.context.apiFetch=async(_method,_path,body)=>{f.writes.push(body);return {ok:true,body:{classifiers:body.classifiers}}};await save.onClick();
