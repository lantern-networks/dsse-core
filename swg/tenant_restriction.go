...
// stages when a later stage fails; failed restoration is reported explicitly.
...
func ApplyTenantRestrictionUpdateContext(ctx context.Context, runtime RuntimeConfig, bundle model.PolicyBundle, store policy.RuntimeStore, req TenantRestrictionUpdateRequest) (TenantRestrictionUpdateResponse, error) {
	...
	attempted := []pendingFile{}
	fail := func(stage string) (TenantRestrictionUpdateResponse, error) {
		for i := len(attempted) - 1; i >= 0; i-- {
			file := attempted[i]
			err := (blobstore.FilePersister{Path: file.path}).Save(file.original)
			if err != nil && !errors.Is(err, blobstore.ErrSavedWithoutAtomicity) {
				stage = "header_restore_unconfirmed"
				continue
			}
			if file.operator {
				resp.PersistedToOperatorConfig = false
			} else {
				resp.PersistedToValueStore = false
			}
		}
	...
	if len(updates) > 0 {
		for i, path := range []string{runtime.TenantRestrictionOperatorConfigPath, runtime.TenantRestrictionOperatorValueStorePath} {
			...
			raw, err := prepareOperatorConfigHeaderValues(original, updates)
			if err != nil {
				return fail("header_file_prepare")
			}
			files = append(files, pendingFile{path: path, raw: raw, original: original, operator: i == 0})
		}
		if len(files) == 0 {
		...
		for _, file := range files {
			...
			if err := (blobstore.FilePersister{Path: file.path}).Save(file.raw); err != nil && !errors.Is(err, blobstore.ErrSavedWithoutAtomicity) {
...
func prepareOperatorConfigHeaderValues(raw []byte, updates map[string]string) ([]byte, error) {
